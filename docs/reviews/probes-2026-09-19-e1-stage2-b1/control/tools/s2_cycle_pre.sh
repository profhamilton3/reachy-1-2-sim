#!/usr/bin/env bash
# Operator wrapper (Stage 2 B1 s1 session): C1 reset -> C2 settle -> C3 parked
...
# B1 change vs the B4 session copy: the board is an argument (was a `B4`
# literal in the parked.sh call) and the cycle id must name that board.
# Usage: s2_cycle_pre.sh <EV> <REPO> <RUN> <board> <cycle> <gen> <route-label>
set -u
EV=$1; REPO=$2; RUN=$3; BOARD=$4; C=$5; GEN=$6; ROUTE=$7
PY=${E1_PYTHON:?set E1_PYTHON}
[ -f "$EV/control/stop" ] && { echo "STOP marker present; refusing"; exit 9; }
case "$C" in S2-"$BOARD"-*) ;; *) echo "cycle id $C does not name board $BOARD; refusing"; exit 9;; esac
...
echo "=== $C C3 parked ==="
bash "$EV/control/tools/parked.sh" "$REPO" "$EV" "$C-parked" "$BOARD" "$ROUTE" "$C" 2>&1 | tee "$EV/control/parkedsh_$C.txt"
