...
clearance per recording / hand model / link / object for every Stage 2 B1
recording (B4 session script with the scene, board objects and flagged cycle
parameterized; B1 has two board objects, aggregated per object), then aggregated by route x role x link across the six repetitions.
...
scene = SceneModel.from_yaml(f"{REPO}/scenes/e1_boards/B1_evidence.yaml")
HANDS = M.HAND_MODES
BOARD_OBJS = ("foam_block", "soda_can")
BOARD_ID = "B1"
SLEEP_AFFECTED_CYCLE = None  # no sleep-affected cycle in the B1 s1 session (caffeinate from before start)
...
def agg(exclude_sleep, BOARD_OBJ):
    rows = []
    groups = {}
...
all_rows = {o: agg(False, o) for o in BOARD_OBJS}
json.dump({"all_flown_reps_by_object": all_rows, "not_flown": ["S2-B1-c-r6 (session stopped at reset 18)"]},
...
lines = ["# Planned minus realised clearance by route x role x hand x link, per B1 board object, across repetitions (cm)", "",
         "Pure geometry from the recorded joint samples at each sample's own measured aperture, against the B1 YAML poses "
         "(`soda_can` r1c1, `foam_block` r2c3; same method as Stage 1's `clearance_by_link.py` and the B4 session's). `realised` for "
         "setups/flights is the **moving** segment (joint velocity > 2 deg/s); parked recordings use the whole 3 s recording. "
         "`delta` = planned - realised: positive means the flight came closer than the planned corridor. n = recordings in the group "
         "(6 per route/role for a/b shapes, 5 for c-shape routes because S2-B1-c-r6 was not flown; 6 or 11 for parked). Zero physics "
         "contacts on every recording; negative clearance is the conservative capsule model overlapping, not a contact. Deterministic "
         "simulator: the repetitions of one route are near-identical re-runs of one open-loop trajectory from one verified reset state, "
         "so agreement across them is evidence of repeatability of this procedure on this board, NOT independent evidence of broad "
         "reliability. No sleep-affected cycle in this session.", ""]
for obj in BOARD_OBJS:
    rows = all_rows[obj]
    lines += [f"## Object `{obj}` -- all flown cycles", "", "| route | role | hand | link | n | planned | realised min..max | delta mean (min..max) | parked-tail min |",
              "|---|---|---|---|---|---|---|---|---|"]
    for r in rows:
    ...
    lines.append("")
# per-recording closest shells link to foam_block for every moving recording
lines += ["## Per recording: closest shells link to `foam_block` (moving segment, cm)", "",
          "| recording | role | closest link | planned | realised | delta |", "|---|---|---|---|---|---|"]
BOARD_OBJ = "foam_block"
for name, rec in sorted(result.items(), key=lambda kv: (kv[0].split('-')[-2] if kv[0].startswith('S2') else '', kv[0])):
    if rec["role"] == "parked" or rec["role"] == "armon":
        continue
    ...
    lines.append(f"| {name} | {rec['role']} | {link} | {fmt(100*plan if plan is not None else None)} | {fmt(100*real)} | "
