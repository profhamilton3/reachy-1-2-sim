"""Offline ledger builder for the Stage 2 B4 s1 session: one row per recorded
...
COLS = ["cycle", "shape", "rep", "session", "role", "leg", "route", "board", "board_sha256",
        ...
        "reset_in_window", "displacement_pool_box_1_m", "displacement_max_m",
...
def row(cycle, shape, rep, role, leg, route, tail_target, tail_mode, tool):
    ...
    if sc:
        ...
        r.update(run_dir=os.path.relpath(sc.get("server_run_dir", ""), EV),
                 ...
                 displacement_pool_box_1_m=f"{disp.get('pool_box_1', float('nan')):.3g}",
                 displacement_max_m=f"{max(disp.values()):.3g}" if disp else "",
                 object_disturbance_gt_1mm=any(v > 1e-3 for v in disp.values()))
...
def main():
    rows = []
    rows.append(row("stage0-B4-s1", "", "", "armon", "stage0-B4-s1-armon", "RAISE_TO_SIDE (label only)",
                    "INIT", "end", "reachy.turn_on('r_arm') (stage0 notebook)"))
    plans = sorted(glob.glob(str(EV / "plan_S2-B4-*.json")),
                   key=lambda p: (int(re.search(r"-r(\d+)", p).group(1)), re.search(r"S2-B4-(\w)-", p).group(1)))
